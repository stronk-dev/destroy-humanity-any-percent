...
| AC2 | **Proven backend contract** | Corrected Q-002 pins every deterministic operation/status error pair and exact response bytes; invalid pair and appended-byte mutations fail. The live registry narrowing landed under separately approved API Foundation authority; both verdicts close at `bfd9b65`. | Preserve the single registry authority and exact byte oracles; this does not claim a player client. |
| AC3 | **Proven integration** | Q-002 drives Recovery heartbeat flooding through stateful authoritative progress/session/token state, proves the rejected seventh heartbeat changes no bytes, and proves refill admits exactly the next transition. Limiter bypass fails; Claude approved at `bfd9b65`. | Preserve before/reject/after/refill ordering. |
| AC4 | **Proven backend contract** | Q-002 enumerates all eight Minigame/Recovery operations, every server-owned identity/clock request field, and hidden response authority. Minigame and Recovery leak mutations fail; Claude approved at `bfd9b65`. | Preserve all-eight enumeration as operations evolve. |
