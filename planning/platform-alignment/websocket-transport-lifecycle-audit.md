...
| AC2 | **Proven integration** | Q-003's production browser persists exact epoch/offset positions, reconnects with recovery, consumes recovered publications through the live path, and full-syncs unavailable history. The composed browser closes a real socket, commits a Postgres intent while disconnected and lands the exact missed receipt/revision. Claude approved `bfd9b65..65b2506` at `249719c`. | Preserve the composed disconnect seam; Account credential rotation remains RP-048. |
| AC3 | **Proven integration** | `TestActualTenSecondStalledWorldConsumerConvergesToLatest` holds a real socket unread for 10.50 s, accepts bounded live catch-up or disconnect, then reconnects from a known position to final revision 93. Two independent latest/recovery-history severings fail. Claude approved at `249719c`, noting write-path boundedness remains independently unit-proven. | Preserve the literal duration, recovery landing and queue unit population. |
| AC4 | **Proven integration** | The server emits typed 4000 for observable receipt overflow; Q-003 maps 4000/invalid-frame closes to authoritative full sync, resets cursors and reconnects. The composed missed-receipt path lands committed state; Claude approved at `249719c`. | Preserve the close-code and resync mutations. |
| AC5 | **Proven integration** | Server drain order remains cold-green. Q-003's production controller honors `resume_after_ms` and uses the shared reconnect/recovery path without duplicate cursor mutation; Claude approved at `249719c`. | Preserve drain delay and shared-path controls; token rotation remains Account-owned. |
| AC6 | **Proven negative integration** | A real second account creates another Guild; the first account's subscription is denied 103 beside the member control, and forcing the production resolver open fails. Denied Match remains; Claude approved at `249719c`. | Add the positive Match participant only when its owner lands. |
