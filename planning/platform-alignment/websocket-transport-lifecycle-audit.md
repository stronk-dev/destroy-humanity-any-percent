...
| AC3 | **Partial** | Queue and history primitives retain only the newest world gauge. The live slow-reader test publishes eight large frames over about 0.9 s and permits one in-flight frame plus the newest (`len <= 2`); the recovery test proves one cached latest snapshot after disconnect. Neither stalls a connected consumer for the literal 10 seconds and requires exactly one snapshot on resume. | Add the exact 10-second connected-stall witness or author-reconcile the criterion to the implementable in-flight-plus-newest contract. The fixture must fail if a stale queued snapshot leaks. |
...
3. Either add AC3's literal 10-second/exact-one witness or have the ruling author reconcile it to
   the already-documented in-flight-plus-newest live contract.
