...
| AC3 | **Owner-reconciled; witness pending** | The 11.29-second probe proved that an exact-one-frame rule is not observable with ordinary browser WebSockets: a blocked connection reaches its byte bound before the post-admission stale hook can run. The 2026-08-21 owner ruling now requires bounded backlog and newest-state convergence through live delivery or typed 4000 recovery, with no lost committed progress. | Add the literal ten-second ruled-outcome witness and a failing recovery/backlog mutation, then include it in Q-003 review. |
...
3. Add AC3's owner-reconciled ten-second witness: bounded live backlog or typed 4000 close, followed
   by newest-authoritative-state convergence without lost committed progress.
