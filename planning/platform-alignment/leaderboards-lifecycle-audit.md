...
- AC1, AC3 and AC6 now have post-coordinate literal, mutation-proven witnesses;
...
- the plan predated the implemented Run Genesis successor at this audit coordinate; its historical
  item and already-archived remediation thread are now reconciled; and
...
| AC3 | **Post-coordinate witness complete; review pending** | The real Prestige crossing fixture now reconstructs and verifies the persisted epoch-N run, supplies N+1 only as the Exit successor catalog, projects the verified run, and proves its row exists only on N's board. Current-hash substitution returns `constants_mismatch`; querying N+1 fails empty. | Include the exact witness range in the mandatory cross-party verdict. |
...
At the audit coordinate no plan box changed. The post-coordinate range reconciles item 3 as
superseded successor ownership without manufacturing a checkbox, while item 8 closes alongside
the exercising AC1/AC3/AC5/AC6 tests in that same review range.
...
3. **AC1/AC3/AC6 complete after this coordinate; designated review remains.**
...
5. **Plan/thread reconciliation complete after this coordinate; the remediation thread was already
   archived and was not moved again.**
