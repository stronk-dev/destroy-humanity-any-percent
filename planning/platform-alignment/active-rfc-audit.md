...
| Account & Session Bootstrap | implementing | Backend scope is extensively implemented and documented; plan/log stop before a modern full-range archival verdict and user account operations have no UI. | Re-walk RFC acceptance and range union; route UI/export gaps to a successor instead of silently widening this RFC. |
...
| Minigame Platform Foundation | accepted / implementing | Persistence, tenant boundary, payout/replay, composition, and production artifact were implemented across this and successors; its plan still reports obsolete blockers and open composition/mint items. | Cross-RFC acceptance reconciliation and range-union audit; do not archive from the stale plan. |
