...
| Declarative content and copy | file/family/shipped-source and 208-key consumption boundary complete; 19-artifact row audit predeclared | All 91 balance and ten copy files plus 23 live/platform families and 208 generated keys have exact producer/consumer verdicts. The measured split is 128 mounted, 63 backend/data-only, one unmounted, eight fixture-only, and eight unreferenced; RP-108/RP-109 route the incomplete registry and four missing current reason keys. The generic JSON-unit grammar now freezes the remaining row denominator before verdicts. |
...
| Planning/docs/shared memory | file/thread boundary complete; author closeout repairs pending | All 23 planning threads, 38 docs, 222 tracked planning files, and 25 ignored planning files are classified; owner/ruling-author body fixes and archival moves remain routed, not inferred. |
