...
| Executable evidence | 802-unit row-oracle structure complete; semantic verdicts pending | All 72 phony targets, seven CI jobs, 56 local client artifacts, 151 server test files, 592 Go functions, 210 client declarations/helpers, and 40 skip sites reconcile. Body/signals are structural only until exact fixture/oracle controls attach. |
...
| Planning/docs/shared memory | file/thread boundary complete; author closeout repairs pending | All 23 planning threads, 38 docs, 233 tracked planning files, and 25 ignored planning files are classified; owner/ruling-author body fixes and archival moves remain routed, not inferred. |
