...
| 2. Capability trace | 432-child structural population complete; evidence attachment pending | For each bounded outcome, trace intent -> producer -> consumer -> real data/workflow -> executable witness. The extractor maps all 121 parents to unique sequential children and demonstrates dropped/duplicate/no-op-split failures. | `capability-outcome-ledger.tsv`, expanded `reality-audit.md` and `capability-map.md` | Every child has exact evidence and route. Structural atomization alone earns no capability verdict. |
...
|---|---|---|
| Design promises and release obligations | 121 parents -> 432 structural children complete; trace verdicts pending | All parents reconcile; 112 split and only nine remain atomic. Mandatory controls cover T2, Founder currencies, minigame contracts, guilds, security, Run End, and content families without granting inherited proof. |
