...
| 2. Capability trace | predeclared; child extraction pending | For each bounded outcome, trace intent -> producer -> consumer -> real data/workflow -> executable witness. `capability-atomization-plan.md` freezes the 121-parent split grammar and mandatory anti-aggregation controls before child verdicts. | `capability-outcome-ledger.tsv`, expanded `reality-audit.md` and `capability-map.md` | Every parent maps to independently falsifiable children; every child has exact evidence and route. No “principal capabilities” sampling remains. |
...
|---|---|---|
| Design promises and release obligations | 121-parent pass complete; child atomization predeclared | Stable parents remain the coverage denominator; mandatory split controls prevent T2, Founder currencies, minigame contracts, guilds, security, Run End, and content families from retaining umbrella verdicts. |
...
| Declarative content and copy | file/family/shipped-source and 208-key consumption boundary complete | All 91 balance and ten copy files plus 23 live/platform families and 208 generated keys have exact producer/consumer verdicts. The measured split is 128 mounted, 63 backend/data-only, one unmounted, eight fixture-only, and eight unreferenced; RP-108/RP-109 route the incomplete registry and four missing current reason keys. |
...
| Planning/docs/shared memory | file/thread boundary complete; author closeout repairs pending | All 23 planning threads, 38 docs, 216 tracked planning files, and 25 ignored planning files are classified; owner/ruling-author body fixes and archival moves remain routed, not inferred. |
