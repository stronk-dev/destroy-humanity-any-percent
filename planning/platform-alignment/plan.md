...
| 3. Acceptance and test audit | execution in progress; Account/Transport/Game UI and Permits/FCE passes complete | Re-walk every active RFC AC and a risk-ranked sample of archived ACs. Inspect fixtures/oracles; run cold gates; add mutation/negative probes only under accepted RFC authority. Record caches, exclusions, guard exhaustion, truncation, and hosted/local differences. | `acceptance-audit.md`, `test-evidence-audit.md`, `permits-first-content-lifecycle-audit.md` | Every current completion claim has a discriminating executable witness or is downgraded/filed. Current-head gates reach valid objectives or fail loudly. |
