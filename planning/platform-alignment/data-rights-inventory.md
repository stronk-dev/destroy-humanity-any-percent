...
| **Transport (1):** `transport_player_outbox` | Transport producer → player socket/recovery delivery. Scope/Founder-linked messages and delivery history remain a distinct disclosure/retention subject; delivery is not by itself proof of row removal. [V] |

The groups are a **relation-complete census**, not a field-by-field personal-data classification.
For the seven unmapped Minigame/Soul/Transport tables, a future tranche must still
classify each SQL column. For all families, a future contract must specify nested
payloads' inclusion and erasure disposition. No group gets a made-up
duration from its package name or apparent lifecycle.

The account/save, history/board, shared catalog/Routes/Commons and Guild SQL-column tranches are in
...
[`data-rights-fields-guild.md`](data-rights-fields-guild.md). Together they cover **53 of 60**
game tables. The remaining seven (Minigame, Soul, Transport), nested JSON/binary/text
...
|---|---|
| Browser `localStorage` | `GameUIRuntime` stores access/refresh tokens, account ID and recovery code under `cloud-clicker.credentials.v1`; a bootstrap idempotency key and per-Founder transport positions are separate keys. `timing.ts` stores local personal-best/split records under a Founder-specific key. There is no mounted player delete/export control, and no observed account-deletion path clearing these keys. This is device-local state, not a Postgres cascade. [V] |
| Encrypted off-host backups | The Deployment bundle/config implements six-hour and pre-upgrade encrypted backups, retention of every six-hour backup for seven days and one daily through day 30, protecting newest valid/unresolved pre-upgrade copies. A deleted account can therefore remain in earlier backup snapshots; the disclosure and any restore-time reapplication rule are open. Local envelope/retention probes exist; exact supported-host R-006 remains unexecuted. [P] |
| Application/security journal | The shipped topology uses persistent journald, unique service tags and a 14-day measurement/budget contract that must alert before pressure can evict evidence early. No raw-IP producer is shipped. This is an operator retention contract, not a demonstrated clean-host observation or a complete product-data policy. Log fields and incident extracts need their own disclosure classification. [P] |
| Private metrics/alerts | Prometheus is private and configured for 30-day TSDB retention; ordinary metric labels are designed to exclude credentials, recovery codes and raw IPs. Alert/metrics volumes remain operator data and clean-host validation is pending. No player-data export claim follows from their existence. [P] |
| Release/operator artifacts | Manifest, release/rollback ledgers, rotation records, backup headers and recovery-identity observations are written under operator-controlled paths by deployment tooling. They can include epoch, bundle and state fingerprints; exact retention/access/disclosure for each artifact is not established by a player account policy. [V for code, P for host operation] |
