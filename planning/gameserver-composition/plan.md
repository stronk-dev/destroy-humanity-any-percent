...
4. [ ] Compose Account/Commons/Guild transport authorization, leaving only `match:*` deny-closed.
5. [ ] Add `cmd/gameserver`, readiness, signal handling, and bounded drain.
6. [ ] Prove the composed binary against real Postgres, including the F4 settlement seam, world
   snapshots, real socket relay, worker ticks, and session GC.
7. [ ] Update canonical docs and pass the repository verification gates.
