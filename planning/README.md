...
One directory per RFC under implementation: `planning/NNNN-slug/` containing:
...
Lifecycle (RFC-0000): created when an RFC moves to `implementing` → on completion, outcomes distilled into `docs/`, RFC marked `implemented`, directory moved to `planning/archive/NNNN-slug/`. **Never deleted.**
