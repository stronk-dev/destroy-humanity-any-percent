...
## 2026-08-05 — shared Founder v18 catalog-parity remediation

Implemented by: Codex.

- Commit `24d55b0` closes the designated review's pet-catalog F1 across every C17 object layer,
  preventing a Go-only acceptance of incomplete pinned pet artifacts during Founder v18 replay.
- The discriminating table test removes one required key from each policy family. The shared kernel
  version is `0.3.49`, and normal root-level `make verify` is green. No artifact was minted and
  nothing was pushed.
