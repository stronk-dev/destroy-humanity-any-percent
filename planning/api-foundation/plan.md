...
- [ ] Add one validation-only exact-JSON arm to schema response descriptors; construction rejects
  empty, unsorted, duplicate, or schema-invalid literals and registry snapshots deep-clone bytes.
- [ ] Bind the four Minigame operations' existing HTTP error mappings to operation/status-specific
  exact bytes without changing any handler output, status, category, detail, generated schema, or
  generated client artifact.
- [ ] Demonstrate rejection of a schema-valid wrong category/detail pair, an appended response byte,
  invalid literal registration, and post-construction caller/snapshot mutation.
- [ ] Update canonical API Foundation docs; run cold focused/root Go, vet, `api-check`, typecheck,
  and sequential Account/Gameserver Postgres populations.
- [ ] Record an exact-range Codex first-filter and hand this API Foundation range plus Q-002's
