# T0–T1 content candidate log

## 2026-08-10 — Claude-side content review of {9d0cc22} — FINDINGS-FIRST (no SHAs pinned)

- **Review by:** Claude (content-lane review; the draft-ratify pattern). **Recorded by:** Claude.

All six SHAs recomputed and matching; economy/routes load green under the REAL loaders cold;
routes byte-diff vs live = exactly the gate insertion; kept epoch-6 bytes verified byte-equal
(resources, policies, progress coordinates, live multiplier sources); formula shapes exact per
design/02 (geometric curves, ×12/×6.5 cadence, 2× rungs with staggering-as-data, role law);
shipped-roles-only verified; Horse Armor compliant (non-purchasable, non-stateful, disclosure
key); gate.t0_to_t1 = 1e5 cash, first-session-crossable (~8-10 min greedy sim); the seven event
kinds all real production kinds; T01-C10 verified real (and RULED accepted, 2514f0c).

**Fix list before ratification (findings-first):**
- **F1 (HIGH, blocking): NO categories candidate** — routes-v1 adds the gate; the composed bundle
  fails leaderboard full_gate_set equality (the FCE-C10 lesson repeating). Add the candidate (one
  sorted insertion) or a named deferral.
- **F2 (MED-HIGH):** three upgrades priced far beyond their availability window's terminal gate
  burn (institutional_memory ~430×, refurbished_sticker 36×, nephew_business_cards 17×) —
  structurally near-dead in-window; will trip the mandatory relevance gate. Rescale or justify.
- **F3 (MED):** beige_tower's 1.13 ratio vs the RFC body's own T0 band 1.07–1.12 — keeping live
  bytes is defensible but the deviation must be surfaced + the body reconciled.
- **F4 (MED):** legal_dept's forced v4 fields CHOOSE synergy_feed into a global pool (a
  Permits-owned tier-3 row now feeding a production multiplier) presented as "preservation" —
  declare as a change for ratification.
- **F5 (MED):** the scenario's "complete invariant set" omits shipped must_reach/resource_bounds
  and invents three names the shipped harness lacks — define the extension contract honestly.
- **F6–F10 (LOW-MED..LOW):** milestone clock basis unpinned (run-2-faster not actually asserted);
  the scripted-failure narrative copy has no named binding surface; provenance stragglers
  (manual/stock ppm, hardcap sentinel, seeds/budget); the v2 base_rate cadence truncation typo +
  the provision-showcase relevance risk; deferral list must name T01-C6/AC0 + the FCE-C8
  orphan-first copy order.
...
## 2026-08-12 — Codex cross-party re-review of `cf70f2d`: NOT APPROVED

- **Review by:** Codex. **Recorded by:** Codex. Reviewed the Claude-co-authored remediation commit
  `cf70f2d`; no implementation code is consumed by this verdict.
- The T01-C18 reachability ruling is internally coherent. The two protocol/body remediations are
  not closed:
  1. **F1 — MEDIUM, wrong forward-correction identity.** The landed repair is
     `kernel/baseline-history-corrections.json` plus guard implementation/registration in
     `{759e369, 0151ddc}` (with `3a3f4cd` correcting the acceptance claim). `a48686d` is the later
     T01-C17 scenario-hash regeneration; it neither isolates d05dc13's original generated bytes nor
     participates in the ledger's validation. The RFC must cite the actual path/range rather than
     declaring `a48686d` the correction.
  2. **F2 — HIGH, stale normative text still remains in place.** The remediation appends a sentence
     saying the earlier `T01-C12 — RULED` paragraph is stale, but leaves that paragraph's exact
     static-ceiling requirements normative in the body. `AGENTS.md` requires the ruling author's
