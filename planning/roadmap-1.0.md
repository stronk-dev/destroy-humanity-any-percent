...
| Account, rights and privacy | Backend security/deletion tests exist; player recovery/export/delete and retention disclosure do not. All 60 game tables have SQL-column maps, but most nested payloads and non-DB fields remain unclassified. The maps distinguish removed credentials from retained Founder/save/intent, archive, boards, shared Route/Commons/Guild and Minigame/Soul/Transport rows. Cold Guild deletion evidence finds a UUID retained in clearing-event JSON despite a nulled account FK (RP-120); the reused-DB Guild suite is not isolated (RP-121). Populated-board deletion remains untested, archived Founders can remain counted in Commons health (RP-119, source-derived), and Founder-linked Minigame/Soul/Transport rows survive deletion by source/FK trace (RP-122). | Owner/legal D-008/D-009/D-015 decisions, nested/non-DB classification and joined rights/Commons witnesses, accepted Account/UI/retention contracts, R-003 on the actual workflow. |
...
3. Bring the source-traced [`Phase-0 exact-manifest proposal`](platform-alignment/phase0-manifest-proposal.md)
   ...
   Finish nested-payload and non-DB rights classification, then
