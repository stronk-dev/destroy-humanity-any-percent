...
| Account, rights and privacy | Backend security/deletion tests exist; player recovery/export/delete and retention disclosure do not. All 60 game tables have SQL-column maps and the four browser-local key families are traced; most nested payloads and operator fields remain unclassified. The browser bootstrap silently persists the one-time recovery code without a player display/copy/download path (RP-123). Cold Guild deletion evidence finds a UUID retained in clearing-event JSON despite a nulled account FK (RP-120); the reused-DB Guild suite is not isolated (RP-121). Populated-board deletion remains untested, archived Founders can remain counted in Commons health (RP-119, source-derived), and Founder-linked Minigame/Soul/Transport rows survive deletion by source/FK trace (RP-122). | Owner/legal D-008/D-009/D-015 decisions, nested/operator-field classification and joined rights/Commons witnesses, accepted Account/UI/retention contracts, R-003 on the actual workflow. |
