...
| Account, rights and privacy | Backend security/deletion tests exist; player recovery/export/delete and retention disclosure do not. The 60-table/non-DB inventory and 53-table SQL-column maps distinguish removed credentials from retained Founder/save/intent, archive, boards, shared Route/Commons and Guild records. Cold Guild deletion evidence finds a UUID retained in clearing-event JSON despite a nulled account FK (RP-120); the reused-DB Guild suite is not isolated (RP-121). Populated-board deletion remains untested, and source trace shows archived Founders can remain counted in Commons health (RP-119). Seven tables, most nested payloads and non-DB fields remain unclassified. | Owner/legal D-008/D-009/D-015 decisions, remaining field/payload disposition and joined rights/Commons witnesses, accepted Account/UI/retention contracts, R-003 on the actual workflow. |
...
3. Bring the source-traced [`Phase-0 exact-manifest proposal`](platform-alignment/phase0-manifest-proposal.md)
   ...
   Finish the remaining Minigame/Soul/Transport and payload/non-DB rights inventory, then
