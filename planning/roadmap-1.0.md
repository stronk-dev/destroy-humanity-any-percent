...
| Account, rights and privacy | Backend security/deletion tests exist; player recovery/export/delete and retention disclosure do not. A current 60-table/non-DB inventory and cold Postgres deletion diagnostic now expose a permanent account-linked bootstrap tombstone that the existing secret-only AC6 check does not count. | Owner/legal D-008/D-009/D-015 decisions, per-column disposition, accepted Account/UI/retention contracts, R-003 on the actual workflow. |
| Accessibility and usability | Current-source Chromium/WebKit probes fail lifecycle focus and 320 px reflow; numeric shell motion still ignores OS preference. Firefox could not start in the local runner. A cross-surface RFC is drafted but not accepted. | Owner task/AT matrix and accepted contract, RP-082–RP-084 repairs under that authority, then R-005/R-008 on built player tasks. |
