...
| DP-F2 | Reproducible exact candidate/previous bundle build, six image SBOMs/config identities, source/image secret scan, retained build ledger and fixed-command driver binding to those exact bundles | implementing; 15/43 populations have real probes, no run claim |
