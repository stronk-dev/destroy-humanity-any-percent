...
1. [ ] Add the strict Guild catalog and schema, register it as an append-only epoch artifact,
   mint the required balance epoch, and prove the epoch/baseline protocol.
2. [ ] Add guild, membership-history, application/invitation, event, projection, XP, Health-input,
   and exchange persistence with migrations and database invariants.
3. [ ] Implement the closed account-scoped lifecycle intent surface with C1-compatible exact
...
5. [ ] Replace the deny-all `guild:*` transport resolver with active-membership authorization and
