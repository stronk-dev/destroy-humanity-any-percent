...
The continuous-time engine: one lane, capacity flow, timed placements, core-kill win. Pure function `(seed, decks, placements) → battle_log`; live-only for the human side (integration RFC owns sessions), opponent = snapshot + bot.
...
- **Capacity:** ×10 fixed-point int (`capacity_dds`), +1 dds per 3 ticks, cap 100 dds (10.0); double-rate in sudden death.

### L2 — Placements and legality

`placement = {tick, card_id (from the 8-card deck), position}`; legal iff `tick` within match, capacity ≥ cost at that tick (evaluated in canonical order, parent C4), position on own half `[0,450]` / `[550,1000]`, card in deck with cycle rule: the played card goes to the back of the 8-card rotation, only the front 4 are playable. Illegal placement = terminal `illegal_action` (same doctrine as the duel engine: callers validate, engines never repair).

### L3 — Resolution per tick (closed order)

`capacity accrual → spawns (this tick's placements, canonical order) → spell effects → targeting (closed rule: nearest legal target by `targets` field, tie → lower position) → movement (unblocked units advance) → attacks (all simultaneous, damage per parent C3, chart multiplier per unit temperaments) → deaths (simultaneous) → tower/core damage → terminal check`. One rounding site per C3; no float velocity — position advances by int speed per tick.
...
3. Capacity/cycle legality negative tests per union arm.
4. Fuzz: 10k random matches, zero invariant violations (position bounds, capacity bounds, simultaneous-death consistency, terminal reached).

## Content successors

- **"Shipping Wars" (solo era-lane):** `design/03 §12b` — same pure engine,
  era decks selected by tier, building counts as C5-pattern scaling inputs, unranked by
  definition (the Fairness Law). Lands as a content RFC once the engine ships.
...
- 2026-08-06: non-normative reference cleanup for publication; no spec change. The deploy
  resource is renamed `elixir` → `capacity` (incl. `elixir_dds` → `capacity_dds`); same mechanic,
  same numbers.
