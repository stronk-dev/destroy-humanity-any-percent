...
export interface CommonsCatalog {
  ...
  readonly collectiveWeightPpm: number; readonly collectiveExponentPpm: number; readonly collapseHealthPpm: number; readonly healthyHealthPpm: number; readonly maximumBonus: string;
...
const keys = ["schema_version", "source_weights", "default_tithe_ppm", "minimum_tithe_ppm", "maximum_tithe_ppm", "guild_health_weight_ppm", "cohort_health_weight_ppm", "server_health_weight_ppm", "collective_weight_ppm", "collective_exponent_ppm", "collapse_health_ppm", "healthy_health_ppm", "maximum_bonus", "health_recovery_ppm_per_hour", "health_decay_ppm_per_hour", "solidarity_window_ms", "cohort_target_size", "cohort_merge_floor", "npc_population_floor", "npc_weight_ppm", "npc_compliance_ppm", "population_tolerance_ppm"] as const;
...
export function parseCommonsCatalog(source: unknown): CommonsCatalog {
  ...
  const catalog: CommonsCatalog = Object.freeze({
    ...
    collectiveWeightPpm: ratio("collective_weight_ppm"), collectiveExponentPpm: boundedInteger(raw.collective_exponent_ppm, "collective_exponent_ppm", PPM, 10 * PPM), collapseHealthPpm: ratio("collapse_health_ppm"), healthyHealthPpm: ratio("healthy_health_ppm"), maximumBonus: raw.maximum_bonus,
...
export function commonsModifier(catalog: CommonsCatalog, healthPpm: number, solidarityPpm: number): string {
  ...
  if (healthPpm > catalog.collapseHealthPpm) { const x = new Decimal(healthPpm - catalog.collapseHealthPpm).div(PPM - catalog.collapseHealthPpm); collective = x.mul(x.pow((catalog.collectiveExponentPpm - PPM) / PPM)); }
