...
export interface PetCatalog {
  ...
  readonly actions: readonly { readonly action_id: string; readonly stat_id: PetStatID; readonly delta_ppm: number; readonly cooldown_attended_ms: number; readonly min_eligible_ppm: number }[];
...
export function parsePetCatalog(source: unknown): PetCatalog {
  ...
  if (root.schema_version !== 1) throw new SyntaxError("invalid pet catalog version");
