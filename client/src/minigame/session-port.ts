...
export function createBrowserMinigameSessionPort(accessToken: () => string, fetcher: Fetcher = fetch): MinigameSessionPort {
  async function call<T>(method: string, path: string, body: unknown): Promise<T> {
    ...
    try {
      ...
      throw new MinigameTransportError(error instanceof Error ? error.message : "minigame request failed");
    }
    let value: unknown;
    try { value = await response.json(); }
    catch { throw new MinigameTransportError(`minigame response was not JSON (${response.status})`); }
    if (!response.ok) throw apiError(response.status, value) ?? new MinigameTransportError(`minigame request failed (${response.status})`);
    return value as T;
  }
  const path = (template: string, parameters: Readonly<Record<string, string>>): string =>
    template.replace(/\{([a-z_]+)\}/gu, (_, name: string) => {
