...
const literalStyle = /(?:#[0-9a-f]{3,8}\b|\brgba?\(|\bhsla?\(|\b(?:0|[1-9]\d*)(?:\.\d+)?m?s\b|\b(?:Arial|Courier|Geneva|Helvetica|Tahoma|Verdana)\b|\b(?:inset\s+)?-?(?:0|[1-9]\d*)px\s+-?(?:0|[1-9]\d*)px)/i;
...
const uiFiles = (await fs.readdir(ui)).filter((name) => name.endsWith(".ts") || name.endsWith(".svelte"));
