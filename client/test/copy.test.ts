...
describe("copy catalog", () => {
  ...
  it("rejects grammar drift before resolution", () => {
    ...
    for (const text of ["**{amount}** {count}", "`{amount}` {count}", "#", ">quoted", "# {amount} {count}", "<!-- {amount} --> {count}", "1) {amount} {count}", "     {amount} {count}", "[{amount}][source] {count}", "| {amount} | {count} |", "<!DOCTYPE html>", "<?xml version=\"1.0\"?>"]) {
