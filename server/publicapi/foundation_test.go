...
func TestRegistryExactJSONIsClosedAndDiscriminating(t *testing.T) {
	operations := testOperations()
	exact := []byte(`{"category":"invalid","detail":"body"}` + "\n")
	operations[1].Responses[1].ExactJSON = [][]byte{bytes.Clone(exact)}
	...
	unknown := []byte(`{"category":"unknown_id","detail":"body"}` + "\n")
	for name, literals := range map[string][][]byte{
		"empty":          {{}},
		"duplicate":      {bytes.Clone(exact), bytes.Clone(exact)},
		"schema-invalid": {[]byte(`{"category":"not_registered","detail":"body"}` + "\n")},
		"unsorted":       {unknown, bytes.Clone(exact)},
	} {
		t.Run(name, func(t *testing.T) {
			invalid := testOperations()
			invalid[1].Responses[1].ExactJSON = literals
			if _, err := NewRegistry(testSchemas(), invalid); err == nil {
				t.Fatalf("%s exact JSON authority accepted", name)
			}
		})
	}
	raw := testOperations()
	raw[1].Responses = []Response{{Kind: ResponseRaw, Status: 200, ContentType: ContentJSON,
		ContentHashHeader: "X-Content-SHA256", ExactJSON: [][]byte{bytes.Clone(exact)}}}
	if _, err := NewRegistry(testSchemas(), raw); err == nil {
		t.Fatal("exact JSON literals accepted on raw response")
	}
}
