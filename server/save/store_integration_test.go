...
func TestStoreIntegrationRevisionLifecycle(t *testing.T) {
	...
	if pending, err := store.PendingPlayerCount(ctx); err != nil || pending != 2 {
		t.Fatalf("pending player messages=%d err=%v", pending, err)
	}
	claimed, err := store.ClaimPlayerOutbox(ctx, 10, 30*time.Second)
	if err != nil || len(claimed) != 1 || claimed[0].FounderID != intentKey.OwnerID || claimed[0].Revision != 2 || claimed[0].MessageKind != "event" {
		t.Fatalf("claimed=%+v err=%v", claimed, err)
	}
	firstClaim := claimed[0].ClaimToken
	if err := store.ReleasePlayerClaim(ctx, claimed[0].ID, firstClaim); err != nil {
		t.Fatal(err)
	}
	claimed, err = store.ClaimPlayerOutbox(ctx, 10, 30*time.Second)
	...
	claimed, err = store.ClaimPlayerOutbox(ctx, 10, 30*time.Second)
	if err != nil || len(claimed) != 1 || claimed[0].MessageKind != "receipt" || string(claimed[0].Payload) != string(result.Receipt) {
		t.Fatalf("receipt claim=%+v err=%v", claimed, err)
	}
	if err := store.MarkPlayerPublished(ctx, claimed[0].ID, claimed[0].ClaimToken); err != nil {
		t.Fatal(err)
	}
	if pending, err := store.PendingPlayerCount(ctx); err != nil || pending != 0 {
		t.Fatalf("pending after player publish=%d err=%v", pending, err)
	}
	...
	if pending, err := store.PendingPlayerCount(ctx); err != nil || pending != 1 {
