// Package relevancepolicy owns the strict, replay-safe relevance policy grammar.
package relevancepolicy
...
func LoadRelevancePolicy(data []byte, catalog *economy.Catalog, routeCatalog *routes.Catalog) (*RelevancePolicy, error) {
	return Load(data, catalog, routeCatalog, true)
}

// Load parses and validates an immutable relevance policy. requireComplete is
// false only for a gate-bounded policy whose exact bytes are pinned by a
// candidate or replay artifact; scenario loaders separately own completeness
// for the slice they measure.
func Load(data []byte, catalog *economy.Catalog, routeCatalog *routes.Catalog, requireComplete bool) (*RelevancePolicy, error) {
