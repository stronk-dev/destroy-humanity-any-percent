...
artifact bytes. Composition fails closed if the current hash, artifact set, Commons/Economy
boundary, moderation data, server identity, or any owned service constructor disagrees.
...
contract. No empty production resolver or duplicate clearing math exists in the binary.
...
SIGINT and SIGTERM begin the transport drain contract: readiness falls, jobs stop, active channels
receive the restart courtesy frame, new intents are rejected, admitted intents and the player
outbox finish, sockets close, and both Centrifuge and HTTP stop under the catalog's 15-second
bound.
...
commits and consumes a real clearing settlement, collects expired session credentials, and drains
to non-readiness. `make test-save-integration` runs it inside the repository's standard Postgres
test topology.
