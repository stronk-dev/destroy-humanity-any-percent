...
- **Ethical%** — the hard, true route: forgo dark-pattern stages, externality dumping, and VC money ("no XP"). A different grammar: **Trust becomes the production stat** (a Trust-scaled stack replacing the forgone multipliers), mutual aid via the **commons** (`05-mmo.md §5` — the prisoner's dilemma that makes it viable), Open Source synergies, and slow compounding. Brutally slow solo; genuinely viable with cooperation; **technically completable by cheese** — and cheese is celebrated (`Ethical% (Glitched)` is a real category; the joke is the point).
- **The game remembers:** morality is founder-scoped and persists across runs. NPCs, story cycles, and even the pet reference what you did in previous companies. There is no memory-wipe purchase.

  > `DESIGN-GAP:` **contradicted by research — owner decision required.** `design/research/morality-systems.md §7.6` argues this line as written **kills the best moment in the design.** A persistent Trust *score* means run 7 opens carrying the accumulated damage of runs 1–6; **by run 15 the ethical route is arithmetically unreachable** and the moral system has become a one-way ratchet with no redemption arc (inFamous's power-lockout failure, compounded across a whole meta-game). The moment we most want — *a canon-route veteran turning around and attempting Ethical%* — becomes impossible for exactly the players most likely to try it.
  >
  > **Proposed fix, which preserves the intent:** persist the **ledger** (dated facts — what you did, which NPCs remember it, what the pet saw), reset the **scores**, reseeded as `clamp(90 − 0.35·Notoriety, 55, 90)`. "The game remembers" stays literally true and gets *sharper*, because memory becomes narrative rather than a debuff. **Nobody ever starts unplayable.**
