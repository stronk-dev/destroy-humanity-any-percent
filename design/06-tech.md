...
| Big numbers | **`break_infinity.js` 2.2.0** client + **hand-written Go `Decimal`** (float64 mantissa + int64 exponent; ported from the same representation). **Golden-vector JSON file asserted by both Go and TS test suites**, including real browser engines. Wire format: **canonical strings, never JSON numbers.** Display: `@antimatter-dimensions/notations`. A balance-harness scale budget gates any future migration to `break_eternity.js` layers. | `break_eternity.js` now (shares exponent and coefficient precision at layer 1; tetration is not designed); math/big.Float (slow, base-2, excess precision); BigInt (no fractional rates) |
