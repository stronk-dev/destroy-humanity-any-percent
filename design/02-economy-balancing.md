...
- **Route Knowledge** — the speedrun meta, fully specified in `08 §6`: earned by executing routes (first-execution grants, repeat grants, collapse-Exit bonuses, region-draft picks); **spends only on route hints** (revealing a registered route's predicate — the in-game strategy guide). Routes themselves must be executed fresh each run and only ever discount/substitute *gates*, never production. The Depletion gate counts **distinct routes executed across the career** (ledger facts, not a pool).
...
- **The moral quantities are two meters + one ledger:** Trust (above) and Soul (§8) as meters, **Externality demoted to a ledger** (dated entries with addresses — see the Thermal Law, `13 §5`), and **p(doom) removed from the moral axis entirely** (it's a pressure meter, `09 §3`). A **CI correlation gate** keeps Trust and Soul decorrelated so content authors can't silently merge them.
- **Derivation rule:** every moral quantity is **derived from the production stack, never awarded for acts** — the difference between farmable Fallout 3 karma and the commons' Enclosure index.
