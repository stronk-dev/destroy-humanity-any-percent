...
- **Planet Integrity:** one global scalar, **100% → 0, only ever decreasing**, driven by every player's harm and compute footprint. ⚠️ `DESIGN-GAP:` **`tile-placement.md §9` argues this should be per-region with a population-weighted global mean — because a single scalar can say *how bad*, but never *where*.** Without per-region resolution the Thermal Law above has nowhere to land, spatial play has no visible consequence, and the map cannot show the one thing our satire is about: that externality has an address. Rendered as landmass desaturation, receding ice, rising haze, night-side sprawl. **Mitigation decelerates the rate; it never reverses the level.**
...
**(b) Datacenter grid as adjacency puzzle (Dorfromantik/Carcassonne).** Within a site, placing racks/substations/cooling scores on **edge matching** — power wants substation adjacency, cooling wants water edges, latency wants network edges — with penalties for hot aisles facing hot aisles. Turns "buy building" into a placement puzzle that **also generates the diorama's layout for free** (the thing you place is the thing you see — GlassBox again).

Both are opt-in depth: an autoplacer exists for players who don't want the puzzle (accessibility + idle-build parity), and the harness verifies autoplace stays within ~10% of hand-placed optimum so it's never a trap.

> `DESIGN-GAP:` **this sentence refutes the mechanic it defends — owner decision required.** `design/research/tile-placement.md` makes the argument from inside our own docs: **an autoplacer guaranteed within ~10% caps the entire skill ceiling of a full spatial editor at ×1.10 — less than one ×1.15 generator step.** A placement puzzle cannot be both accessible *and* worth building **while its output is a single scalar.**
>
> **Recommendation: cut the rack grid, keep the region draft.** They are the same mechanic at two zoom levels and the rack level is the wrong one — invisible to other players, an *active* system bolted to an *idle* spine, and minigame #11 in a game that already has ten. Revisit post-1.0 as an output-neutral optimiser toy.
>
> **The escape from the 10% trap is Opus Magnum's, not Carcassonne's:** score placement on **three non-commensurable axes (Power / Thermal / Reach)**. Then there is no "10% behind" — there is a Pareto frontier, the community optimises *positions* rather than converging on *the* layout, and the autoplacer becomes trivial (emit any non-dominated point) instead of an NP-flavoured guarantee. **This is the third dossier to reach the same structural law by a different route** — `creature-battler.md` (coverage moves) and `lane-pusher-design.md` (Nash support 1) both found that a single-scalar outcome collapses into a solved problem, and that decorrelated axes are what restore a gradient.
>
> **The Thermal Law is the payoff, and it belongs in `02` as much as here:** `Output = base × min(1, P/P_req)` but `Externality = base × (1 + 0.25·max(0, T_req − T))`. **Under-power a site and it visibly throttles. Under-cool it and it does *not* throttle — it leaks onto the neighbouring region.** The constraint that binds you throttles you; the constraint that doesn't bind you lands on someone else. That is the externality axis as a placement rule instead of a hidden counter, and **it is never explained by a tooltip** — the player places, watches Output hold steady while the region browns, and works it out.
