...
- **Economy hooks:** wins pay Clout + small cash scaled to tier; achievements (beat the top bot, win N ranked games) feed Clout; a daily-puzzle streak feeds a small multiplier. Bot matches pay reduced, non-ranked rewards (anti-farm).
  ...
  > `DESIGN-GAP:` **streak-fed multipliers violate our own law — here and in §7's typing minigame** (`liveservice-idle-tier.md §9.2`). A streak that breaks on absence is *FOMO with a real cost* (`design/00` law 6): miss a day, lose the multiplier, and absence has been priced. The live-service tier's own de-fanged pattern fixes it: **streaks count up and never reset** (lifetime totals with milestone chests), or the multiplier keys on *cumulative* days played. Same daily pull, zero absence tax. Related, larger gap from the same dossier: **nothing in our thirteen design docs owns a daily/weekly session scaffold at all** — the activity-bar-with-designed-"done"-state is the tier's best player-respecting craft (the 100-point chest is a session *terminator*), and it currently has no home. Candidate owner: `design/02` (clocks) or a new §here.
