...
**Advisor Mode (the God-Mode analogue, deferred from the Phase-0 preview by D-012):** the following accessibility design and authored label remain a later target; no current player intent or settings control enables it. A future accepted successor must own its activation contract and surface. When adopted, it is off by default and honestly labeled — *"+2% production per completed run while enabled, up to +50%. For players who want the story at their own pace."* Curtain-pulled like everything else; runs with it enabled carry the `Assisted` variable (`05 §6`) and rank on Assisted boards. No shame chrome, no nag to turn it off — the label states, the boards categorize, nobody judges.
