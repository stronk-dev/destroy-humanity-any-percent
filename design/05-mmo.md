...
- **Micro-reciprocity (the warm channel — `research/liveservice-idle-tier.md §5`):** help taps (one tap shaves a visible sliver off a guildmate's timer, attributed), directed donations with names on them, and **one scheduled 30-minute guild co-presence ritual per week** (booked, not ambient — the calendar creates the appointment, the tithe does the economics). The tithe kills leech resentment; this is what makes a guild feel *inhabited*.
  ...
  > `DESIGN-GAP:` **the ritual has no venue** (`research/social-spaces.md`, 2026-07-28). This weekly co-presence moment is committed design with nowhere to happen. The researched fix — verdict: *build one room, not a world* — is the **Guild Break Room**: one seat-based room docked on this guild page (cap 12–20 so fullness is reachable), founder avatars idle while online, pets + labeled NPC co-op avatars as ambient population (the AI-fallback law answering the empty-room problem; the office fiction absorbs the rest — an empty break room at 4 a.m. is *true*, not sad), emote wheel + curated era-voiced phrase packs, **no free text ever** (the chat ladder stops at rung 2 — the same curate-offline posture as the ticker, with three regimes' worth of evidence: Sky's 260M downloads on wordless warmth, Club Penguin's iceberg myth formed entirely inside menus, and every rung above phrases being a staffing cliff). The walkable hub stays rejected; the furni economy is satirized (`04 §4`), never built. v1.0-minimal *because the ritual already is*.
