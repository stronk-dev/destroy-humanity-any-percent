...
- **Micro-reciprocity (the warm channel — `research/liveservice-idle-tier.md §5`):** help taps (one tap shaves a visible sliver off a guildmate's timer, attributed), directed donations with names on them, and **one scheduled 30-minute guild co-presence ritual per week** (booked, not ambient — the calendar creates the appointment, the tithe does the economics). The tithe kills leech resentment; this is what makes a guild feel *inhabited*.
  ...
  **The Guild Break Room (designed 2026-07-28, from `research/social-spaces.md` — the ritual's venue):**
  - **One seat-based room per guild**, docked on this guild page. Seats, not floors: **cap 12–20, so fullness is reachable** — a full room is an event, and the cap is why.
  - **Ambient population by construction:** founder avatars idle in the room whenever their founder is online (presence made visible, zero effort); bonded pets wander in and lounge; **labeled NPC co-op avatars** fill low-population hours per the AI-fallback law. The office fiction absorbs the rest — *an empty break room at 4 a.m. is atmospherically true, not sad*; after-hours emptiness art-directs into quiet.
  - **Communication: emote wheel + curated era-voiced phrase packs. No free text, ever.** The chat ladder stops at rung 2 (the ticker's curate-offline posture, now with the genre's own evidence: Sky's 260M downloads on wordless warmth; Club Penguin's iceberg myth — a twelve-year collective legend — formed entirely inside phrase menus; every rung above phrases is a staffing cliff). Phrase packs are content-pack data (`12`), era-voiced, seasonal. The constraint doubles as period flavor: menu chat *is* the era.
  - **Scheduled moments carry the live load** (the Pawn-revival lesson: concentrate a thin population into one hour): the weekly co-presence ritual above is *staged here*; Arcade Night and tournament viewing join the same room calendar (`09`).
  - **Never in this room:** a furni scarcity economy (it is the satire *target* — `04 §4` parodies artificial scarcity better than a real one could), free-text chat, room instancing (one room per guild, period — sharding kills place-ness), or purchases of any kind. Decor grows from guild achievements, not currency.
  - Solo/guildless players get the **cohort lounge** — same room system keyed on the cohort (`§5`), so the 150 named neighbors have somewhere to stand.
  - The walkable single-player hub remains rejected (`11 §3`); this is a presence surface, not a narrative space — nobody walks, avatars *are* where their founders are.
...
- **The front door (the discovery surface the research warned we lacked).** The commons is **encountered, not found**: every incorporation (the Tier-2 faction moment, every run) includes a line-item in the contract itself — **"Industry Mutual Aid Compact — [sign] [decline]"** — with its one compliance term stated plainly: *benefits scale with practices, derived from your production stack; formula attached.* Declining leaves a persistent greyed line on the company screen ("Non-signatory"), so the door stays visible all run. The join moment changes **exactly one line** of the production panel — `Mutual Aid: ×1.31 (H = 0.83)` — and opens the cohort panel: your ~150 named neighbors, their co-ops, their standing. Ambient surfaces keep it alive for non-members: the Health dial sits beside Planet Integrity on the map, **dispatches cover commons events server-wide** (defection cascades and recoveries are news everyone reads), and if a player reaches mid-Tier 3 unsigned, one NPC co-op sends a single recruiting event ("The co-op across the street shares their build pipeline. Weird. They seem fine.").
- **Signing while canon is allowed, graded, and the satire writing itself.** Membership is open to all; a dark-pattern-running signatory's Enclosure index marks them a defector from the first tick — low `sᵢ`, buff floor, cohort standing visible. Not a trap: the compact's term was stated at signature. A company signing a mutual-aid compact while enshittifying, publicly graded by its own production stack, *is* the mechanic being satirical without a single line of extra copy.
- **One commons.** `10 §1`'s Open Source faction verb ("contribute to the server-wide commons pool you also draw from") is **this pool** — their faction identity is heavier participation in the same compact (auto-signed at incorporation, larger tithe, Libraries produced through it), not a second object.
