...
| RP-111 | `make release-secret-scan` forwards a relative `SECRET_SCAN_OUTPUT` path as relative even though its structured result writer requires an absolute path; the otherwise valid source/image scan exits 1 with a generic content error. An explicit absolute path succeeds. | ⚠ Deployment tooling usability defect -> accepted Deployment Foundation follow-up; not a failed secret finding or R-006 proof |
| RP-112 | Rollback validation compares a retained previous manifest's save versions to the validator binary's current `save.LatestCompanyVersion`/`LatestFounderVersion`. This pair passes today only because the versions match; a future save-schema bump would reject an otherwise exact previous bundle within its seven-day rollback window. | ⚠ future compatibility risk -> Deployment versioned rollback contract and a cold mixed-version fixture before any save-version bump |
