...
| RP-111 | `make release-secret-scan` forwarded a relative `SECRET_SCAN_OUTPUT` path as relative even though its structured result writer requires an absolute path. | 📜 corrected locally in Deployment Foundation; relative positive and no-overwrite negative pass; designated review pending |
| RP-112 | Rollback validation compared a retained previous manifest's save versions to the validator binary's current `save.LatestCompanyVersion`/`LatestFounderVersion`, risking rejection after a save-version bump. | 📜 corrected locally in Deployment Foundation; mixed-version positive and zero/future negatives pass; designated review and exact clean-host rollback pending |
...
| RP-116 | The Mac Vitest/Playwright Firefox session failed to connect twice within 60 s, initially leaving that current-source probe arm unexecuted. | ✅ product-observation gap closed: declared Linux `make test-browser-ci` ran Firefox and reproduced both defects; Mac launcher remains an environment note, while R-005/manual AT remain open |
| RP-117 | The initial 60-table inventory called the whole Run/Founder family append-only/immutable, overlooking that verified `run_log` rows compact into a full immutable archive and FK-referenced live rows may remain. | ✅ inventory wording corrected and 14-table field map records both storage forms; `planning/platform-alignment/data-rights-fields-history-boards.md` |
| RP-118 | A cold joined HTTP deletion probe now covers one directly seeded verified board row (RP-130). The full producer→archived log→verification/dead-letter→board→account-delete population, public reader and backup restore remain untested together, so the complete retained post-delete outcome is not proved. | ⚠ narrowed integrated rights-evidence gap -> D-009/D-015, later accepted Account/retention witness; `planning/platform-alignment/data-rights-board-delete.md` |
| RP-119 | A cold joined Account API deletion probe began with an active Commons Founder, then archived its stream through deletion while the exact World member-count and `refreshScope` sample-selection SQL predicates still returned 1/1. Setting `member=false` in a rollback-only negative control dropped both to zero. The full projector→delete→world/health workflow and active-vs-historical policy remain absent. | 🔬 verified live-count/sample-selection defect -> D-009/D-015 and accepted Commons/Account successor; `planning/platform-alignment/data-rights-commons-delete.md` |
| RP-120 | A cold Guild deletion diagnostic found one retained `exchange_cleared` event with `actor_account` nulled but `payload.producer_account_id` still equal to the deleted UUID. Source also permits account IDs in clearing allocations; FK anonymization is incomplete for nested JSON. | 🔬 verified payload-retention gap -> D-009/D-015 and accepted Guild/Account successor; `planning/platform-alignment/data-rights-fields-guild.md` |
| RP-121 | The unmodified three-test Guild Postgres integration suite passes cold but fails on immediate rerun against the same database (two invalid intents, then duplicate account primary keys); a DB reset restores green and a second warm rerun reproduces failure. Fixed fixture IDs survive between runs. | ⚠ test-isolation defect -> accepted Guild test-harness repair; `planning/platform-alignment/data-rights-fields-guild.md` |
| RP-122 | Account deletion archives Founder mappings/save streams instead of deleting them; Minigame sessions/commands/receipts/faucet windows, Soul recovery sessions and Transport player outbox rows reference those retained entities, so their cascade FKs do not remove these rows on account deletion. A cold joined Account API probe (2026-09-24) seeded an active Minigame session with create/command receipts, a faucet window and an active Soul session, plus the real intent's receipt/outbox rows: deletion returned 204 and all eight families (including the Soul progress token) remained 1/1; rollback-only controls showed the queries observe removal and RP-128 still blocks parent-session delete. No composed-coordinator or retention policy exists. | 🔬 verified data-rights boundary (seeded rows) -> D-008/D-009/D-015 and accepted Account/retention successor; `planning/platform-alignment/data-rights-fields-final.md`, `data-rights-minigame-soul-delete.md` |
| RP-123 | The browser bootstrap stores the supposedly one-time recovery code indefinitely in `cloud-clicker.credentials.v1` with access/refresh tokens, and the current Game UI has no display/copy/download consumer for it. Client tests confirm storage but not the ruled recovery workflow or local deletion. | ⚠ source/test-confirmed credential-posture gap -> D-005/D-009/D-015 and accepted Account/Game UI successor; `planning/platform-alignment/data-rights-browser.md` |
| RP-124 | The archive-compaction integration fixture logs `{}` command/replay/receipt payloads and expects zero archived events. It proves compression/rollback/immutability but cannot falsify dropped nontrivial payload fields or matched Company/Founder event content. | ⚠ acceptance-evidence gap -> accepted replay/rights witness with populated event and severing case; `planning/platform-alignment/data-rights-payload-core.md` |
| RP-125 | The backup policy protects the newest valid and unresolved pre-upgrade backups beyond the seven/30-day rotation, so 30 days is not a hard account-data maximum. Restoring a full pre-deletion dump can recreate a deleted account; no post-restore deletion replay is composed. The existing backup/restore integration test does not exercise delete-after-backup. | ⚠ source-derived deletion/restore risk -> D-009/D-015 and accepted Account/Deployment successor with joined witness; `planning/platform-alignment/data-rights-operator.md` |
| RP-126 | Append-only release and key-rotation ledgers record a caller-supplied `operator` identifier and have no adopted expiry/access/disclosure rule for that potentially personal operator data. | ⚠ operator-data retention gap -> D-015/ops policy; `planning/platform-alignment/data-rights-operator.md` |
| RP-127 | `soul_recovery_sessions.progress_token` remains non-null on terminal rows: a cold Postgres fixture left raw UUID tokens in all 14 cancelled/resolved sessions, while terminal receipts omit them. Account deletion retains the Founder/session path; no cleanup or disclosure rule exists for this historic capability material. The token alone cannot progress a terminal session and the route also requires authentication. | 🔬 verified retained-token boundary -> D-009/D-015 and accepted Soul/Account retention contract; `planning/platform-alignment/data-rights-receipts.md` |
| RP-128 | `00071`'s unconditional Minigame API receipt `BEFORE DELETE` trigger blocks even FK cascade from deleting a parent session with a create/command receipt. A transaction-scoped cold Postgres parent delete failed with `minigame API receipts are immutable` and rolled back. Session-expiry cleanup cannot be implemented by parent deletion without a migration/contract change. | 🔬 verified retention-cleanup blocker -> D-015 and accepted Minigame retention successor; `planning/platform-alignment/data-rights-receipts.md` |
| RP-129 | The verification retry/poison path stores bounded but unredacted upstream `err.Error()` text in `verification_queue.last_error`, immutable `verification_poison_dead_letters.detail`, and an invariant report. A cold injected-failure test persisted its synthetic error text exactly. This proves arbitrary diagnostic pass-through, not an actual secret leak; upstream error sources and retention/disclosure need a ruled boundary. | ⚠ verified diagnostic-data boundary -> D-009/D-015 and accepted verifier/operations privacy contract; `planning/platform-alignment/data-rights-verification.md` |
| RP-130 | A cold real-Postgres Account API deletion probe with a populated, schema-valid board row succeeded: the account vanished, while `verified_runs.founder_id` still joined to an archived/unlinked Founder with two archived streams. This is a relational retention fact, not proof that a public board exposes identity or that the retention is lawful/ruled. | 🔬 verified joined board/Founder retention -> D-009/D-015 and accepted Account/Leaderboards rights contract; `planning/platform-alignment/data-rights-board-delete.md` |
