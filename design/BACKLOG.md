...
| RP-119 | Account deletion archives Founder streams without clearing `company_compact_memberships.member`; Commons `refreshScope` counts stored samples where `member=true` without checking stream archival, so a departed Founder can still affect later cohort/server health recomputation. No joined delete→refresh witness or active-vs-historical policy exists. | ⚠ source-derived integration/retention risk -> D-009/D-015 and accepted Commons/Account successor; `planning/platform-alignment/data-rights-fields-shared.md` |
